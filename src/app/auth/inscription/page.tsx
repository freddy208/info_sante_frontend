...
import { Eye, EyeOff, UserPlus, Heart, Sparkles, ArrowRight, Zap, Loader2, Mail, Lock, User, Phone, MapPin, ChevronDown } from 'lucide-react';
import { useAuth } from '@/hooks/useAuth';
import { registerSchema, RegisterFormData, CameroonRegion, CameroonCity } from '@/lib/validations/auth';
import { toast } from 'react-hot-toast';

// Association des villes par région
...
export default function InscriptionPage() {
  ...
  const [availableCities, setAvailableCities] = useState<CameroonCity[]>([]);
  ...
  // Mettre à jour les villes disponibles lorsque la région change
  useEffect(() => {
    if (selectedRegion) {
      setAvailableCities(regionCities[selectedRegion] || []);
      ...
      >
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-100 p-6 sm:p-8 mb-6">
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <select
                    ...
                    className={`w-full pl-12 pr-12 py-4 bg-gray-50 border-2 rounded-2xl text-gray-900 placeholder-gray-400 focus:bg-white focus:ring-4 focus:ring-teal-500/10 transition-all outline-none font-medium appearance-none ${
            ...
            {/* Bouton d'inscription avec bleu santé doux et gradient harmonieux */}
            <button
              type="submit"
              disabled={isLoading}
        ...
        <div className="text-center space-y-4">
          <p className="text-gray-600">
            Déjà un compte ?{' '}
            <Link href="/auth/connexion" className="text-teal-600 hover:text-teal-700 font-bold transition-colors hover:underline inline-flex items-center gap-1 group">
              Se connecter
        ...
        {/* Footer premium adapté PWA *
        <div className="pt-8 sm:pt-12 flex flex-col items-center gap-3">
          <div className="flex items-center gap-2 text-gray-700">
            <Heart className="w-5 h-5 text-red-500 animate-pulse" />
            <p className="text-sm sm:text-base font-semibold">Conçu avec passion pour le Cameroun 🇨🇲</p>
          </div>
