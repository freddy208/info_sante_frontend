...
import { 
  ...
  ChevronDown, ChevronUp, Copy, Check, Phone, Globe,
...
// ✅ IMPORTS FAVORIS (BOOKMARKS)
import { useIsBookmarked, useToggleBookmark } from '@/hooks/useBookmarks';

// ✅ IMPORT DES UTILITAIRES DE COULEUR
...
// ==========================================
// 🗺️ INTERACTIVE MAP (CORRIGÉE)
// ==========================================
function InteractiveMap({ location, organizationName }: { location: any; organizationName: string }) {
  const [isClient, setIsClient] = useState(false);
  const [isMapReady, setIsMapReady] = useState(false); // État pour s'assurer que la carte est prête
  ...
  // Sécurité supplémentaire : Attend que le MapContainer soit monté
  useEffect(() => {
    if (isClient) {
      // Petit délai pour s'assurer que le DOM de la carte Leaflet est prêt
  ...
  return (
    <div className="w-full h-64 md:h-80 z-0">
      ...
      >
        {/* ✅ CORRECTION : On ne rend les couches QUE si la carte est prête */}
        {isMapReady && (
          <>
...
function HeroImageSection({ 
  ...
  return (
    <div className="relative w-full h-[60vh] min-h-80 sm:h-[70vh]">
      ...
      {/* ========================================== */}
      {/* 💡 CHANGEMENT : BADGE PLACÉ EN BAS À DROITE */}
      {/* ========================================== */}
      {statusBadge && (
      ...
      {/* Conteneur Texte (Gauche, Bas) */}
      <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8 z-10">
        {/* On garde une marge à droite si besoin, mais le badge est en absolute, donc ça va */}
      ...
      <div className="bg-white rounded-3xl p-4 sm:p-6 md:p-8 shadow-xl">
        ...
        <section className="flex flex-wrap items-center justify-between gap-4 mb-8 bg-gray-50 rounded-2xl p-4 sm:p-6">
          ...
          >
             ...
             <span className="text-sm font-medium hidden sm:inline">{isLiked ? 'J\'aime' : 'J\'aime'}</span>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-linear-to-br from-blue-50 to-teal-50 rounded-2xl p-6 border border-blue-100">
             ...
             <p className="text-gray-600 mb-4">{announcement.location?.address}, {announcement.location?.city}, {announcement.location?.region}</p>
...
// ✅ MODIFICATION : CommentsSection reçoit maintenant isAuthenticated
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-2">
                      <h3 className="font-bold text-gray-900 text-sm">Utilisateur</h3>
                      ...
                      <div className="mt-3 bg-gray-50 p-3 rounded-lg">
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-1 bg-teal-600 text-white rounded text-xs disabled:opacity-50"
                          >
                            Répondre
                          </button>
                        </div>
      ...
      >
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-600" placeholder="Votre nom" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Téléphone *</label>
            <input type="tel" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-600" placeholder="+237..." />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Email (optionnel)</label>
            <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-600" placeholder="email@exemple.com" />
          </div>
        </div>
        <footer className="mt-6">
          <button onClick={handleSubmit} className="w-full py-3 bg-linear-to-r from-teal-600 to-blue-600 text-white rounded-xl font-bold shadow-md">Confirmer mon inscription</button>
        </footer>
      </motion.div>
    </div>
  );
...
function SimilarAnnouncements({ currentCategoryId, currentId }: { currentCategoryId?: string; currentId: string }) {
    ...
    <section className="max-w-7xl mx-auto px-4 mt-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
        {similar.map((campaign) => (
           <div key={campaign.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all">
              <div className="h-40 relative">
                 <img src={campaign.thumbnailImage || campaign.featuredImage} alt={campaign.title} className="w-full h-full object-cover" />
              ...
              <div className="p-4">
                 ...
                 <button onClick={() => window.location.href = `/annonces/${campaign.slug}`} className="w-full mt-3 py-1.5 bg-teal-50 text-teal-700 rounded-lg text-sm font-bold hover:bg-teal-100">Voir détails</button>
              </div>
           </div>
        ))}
      </div>
...
export default function AnnouncementDetailPage() {
  ...
  // ✅ ÉTATS POUR L'AUTHENTIFICATION ET LE LIKE
...
/**
 * Génère ou récupère un ID unique pour l'appareil.
 * Cela permet de garantir qu'un visiteur ne peut s'inscrire qu'une fois
 * même s'il change son nom ou son numéro.
 */
const getOrCreateDeviceId = (): string => {
  ...
  if (!deviceId) {
    ...
    // Note: crypto.randomUUID() est supporté par les navigateurs modernes (HTTPS ou localhost)
    if (typeof crypto !== 'undefined' && crypto.randomUUID) {
      deviceId = crypto.randomUUID();
    } else {
      // Fallback pour anciens navigateurs (peu probable)
      deviceId = 'device-' + new Date().getTime() + '-' + Math.random().toString(36).substr(2, 9);
    }
  ...
  // ✅ HOOKS FAVORIS (BOOKMARKS)
  const { data: bookmarkCheck } = useIsBookmarked(ContentType.ANNOUNCEMENT, announcement?.id || '');
  const toggleBookmark = useToggleBookmark();

  // État dérivé pour le favori (fallback sur false si pas de données)
  ...
  const handleBookmarkToggle = () => {
    ...
    // 2. Appel l'API via le hook (Gère Ajout / Suppression automatiquement)
  ...
  // ==========================================
  // GESTIONNAIRES D'ÉVÉNEMENTS (Inscriptions)
  // ==========================================
  ...
  const handleRegistrationSubmit = (data: { visitorName: string; visitorPhone: string; visitorEmail?: string }) => {
    ...
    // 2. Préparer les données à envoyer au backend
    const payload = {
      ...data, // visitorName, visitorPhone, visitorEmail
      deviceId: deviceId, // ✅ On envoie toujours le deviceId
    };

    // 3. Appeler le Hook
    registerMutation.mutate({ id: announcement.id, data: payload }, {
      onSuccess: () => { 
        setIsRegistrationModalOpen(false); 
        setIsConfirmationModalOpen(true); 
        setIsInterested(true); // Mettre à jour l'UI "Inscrit"
        
        // ✅ Invalidation pour mettre à jour les compteurs visibles sur la page
        queryClient.invalidateQueries({ queryKey: ['announcements'] });
        queryClient.invalidateQueries({ queryKey: ['announcements', 'detail', announcement.id] });
      },
      onError: (error: any) => {
        // L'erreur sera gérée par le toast dans le hook (useRegisterAnnouncement)
        // Mais on peut fermer la modale si on veut
        // setIsRegistrationModalOpen(false);
      }
    });
  };
  ...
  const handleInterestToggle = () => {
    if (!isInterested) { setIsRegistrationModalOpen(true); } else { setIsInterested(false); toast.success('Inscription annulée'); }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="w-full pt-8 sm:pt-12">
         ...
         <SimilarAnnouncements currentCategoryId={announcement.categoryId} currentId={announcement.id} />
      </div>
      ...
      {/* ✅ MODALE DE CONNEXION GLOBALE (Utilisée pour les Likes et les Commentaires et les Favoris) */}
      <AnimatePresence>
        {isLoginPromptOpen && (
