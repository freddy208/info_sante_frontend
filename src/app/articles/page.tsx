...
import { reactionsApi } from '@/lib/api-endponts'; // ✅ AJOUT IMPORTANT
...
function ArticleCard({
  ...
  const handleLikeClick = (e: MouseEvent<HTMLButtonElement>) => {
    ...
    setIsLiked(!isLiked); // Animation optimiste
    onLike();
  };
    ...
    >
      ...
      <div className="relative w-full sm:w-48 sm:h-40 h-56 shrink-0">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2">
          <span
            className={`inline-flex items-center px-2 py-0.5 text-[10px] sm:text-xs font-bold rounded-full bg-linear-to-r ${getCategoryColor(article.category?.name || '')} text-white shadow-sm`}
          >
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-1 justify-between">
        <div>
          ...
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] sm:text-xs font-medium text-teal-600 uppercase tracking-wide">
              {article.category?.name}
            </span>
            <div className="flex items-center text-gray-400">
              <Clock className="h-3 w-3 sm:h-3.5 sm:w-3.5 mr-1" />
              <span className="text-xs">{article.readingTime || 5} min</span>
            </div>
          </div>
          ...
          <div className="flex items-center mb-4">
            <div className="relative w-6 h-6 sm:w-8 sm:h-8 bg-gray-100 rounded-full flex items-center justify-center mr-2.5">
              {article.organization?.logo ? (
                <img src={article.organization.logo} alt="Logo" className="w-full h-full object-cover rounded-full" />
              ) : (
                <User className="h-4 w-4 sm:h-5 sm:w-5 text-gray-400" />
              )}
              {article.organization?.isVerified && (
                <div className="absolute -bottom-0.5 -right-0.5 bg-blue-500 rounded-full p-0.5 sm:p-1 border border-white">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-gray-900 truncate">
                {article.author || article.organization?.name}
              </p>
    ...
    >
      <div className="relative h-40 sm:h-48">
        ...
        <div className="absolute top-2 left-2 flex gap-1.5">
          <span className={`inline-flex items-center px-2 py-0.5 text-[10px] font-bold rounded-full bg-linear-to-r ${getCategoryColor(article.category?.name || '')} text-white shadow-sm`}>
      ...
      <div className="p-3 sm:p-4 flex-1 flex flex-col">
        ...
        <div className="flex items-center mb-3">
          <div className="relative w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center mr-2">
            {article.organization?.logo ? (
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-gray-900 truncate">{article.author}</p>
          </div>
        </div>
        ...
        <div className="mt-auto pt-3 border-t border-gray-50">
          ...
          <button
            onClick={handleViewArticle}
            className="w-full py-1.5 bg-teal-50 text-teal-700 text-xs font-medium rounded-full hover:bg-teal-100 transition-colors flex items-center justify-center"
          >
            Lire l&apos;article
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          <button
            ...
            className="flex items-center px-4 py-2 bg-linear-to-r from-teal-500 to-teal-600 text-white text-sm font-medium rounded-full hover:from-teal-600 hover:to-teal-700 shadow-md"
          >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
      ...
      <p className="text-gray-600 mb-6 max-w-md">
        {searchTerm ? `Aucun résultat pour "${searchTerm}". Essayez d'autres mots-clés.` : "Aucun article ne correspond à vos critères actuels."}
      </p>
...
export default function ArticlesPage() {
  ...
  // 1. FAVORIS (FIXE POUR 401/429)
  // ✅ CORRECTION : On passe l'objet options en 2ème argument
  ...
  // ✅ CORRECTION : On appelle le hook pour récupérer l'objet
  const toggleBookmark = useToggleBookmark();

  // 2. LIKE (FIXE POUR 'toggleLikeMutation introuvable')
  // ✅ CORRECTION : On crée la mutation ici car elle n'existe pas
  ...
  // 4. CONSTRUCTION DES PARAMÈTRES API
  const queryDto = useMemo<QueryArticleDto>(() => {
    const dto: QueryArticleDto = {
      page,
      limit: 10,
  ...
  const displayArticles = useMemo(() => {
    ...
    if (filters.readingTime === 'short') {
      filtered = filtered.filter(a => (a.readingTime || 0) < 5);
    } else if (filters.readingTime === 'medium') {
      filtered = filtered.filter(a => (a.readingTime || 0) >= 5 && (a.readingTime || 0) <= 10);
    } else if (filters.readingTime === 'long') {
      filtered = filtered.filter(a => (a.readingTime || 0) > 10);
    }

    const now = new Date();
    if (filters.date === 'week') {
      const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
      filtered = filtered.filter(a => new Date(a.publishedAt || now) >= weekAgo);
    } else if (filters.date === 'month') {
      const monthAgo = new Date(now.setMonth(now.getMonth() - 1));
      filtered = filtered.filter(a => new Date(a.publishedAt || now) >= monthAgo);
    } else if (filters.date === 'year') {
      const yearAgo = new Date(now.setFullYear(now.getFullYear() - 1));
      filtered = filtered.filter(a => new Date(a.publishedAt || now) >= yearAgo);
    }
  ...
  // ✅ CORRECTION : Utilise le hook 'toggleBookmark' récupéré plus haut
  const handleBookmarkToggle = (contentId: string) => {
    toggleBookmark.mutate(ContentType.ARTICLE, contentId, bookmarkedIds.has(contentId));
  };

  // ✅ CORRECTION : Utilise la mutation créée plus haut
        ...
        >
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setFilters({ ...filters, readingTime: filters.readingTime === 'short' ? undefined : 'short' })}
                className={`flex-1 py-2 rounded-lg text-sm border transition-colors ${filters.readingTime === 'short' ? 'bg-teal-50 border-teal-600 text-teal-700' : 'bg-white text-gray-600'}`}
              >
                Rapide ( 5 min )
              </button>
              <button
                onClick={() => setFilters({ ...filters, readingTime: filters.readingTime === 'medium' ? undefined : 'medium' })}
                className={`flex-1 py-2 rounded-lg text-sm border transition-colors ${filters.readingTime === 'medium' ? 'bg-teal-50 border-teal-600 text-teal-700' : 'bg-white text-gray-600'}`}
              >
                Moyen (5-10 min)
              </button>
              <button
                onClick={() => setFilters({ ...filters, readingTime: filters.readingTime === 'long' ? undefined : 'long' })}
                className={`flex-1 py-2 rounded-lg text-sm border transition-colors ${filters.readingTime === 'long' ? 'bg-teal-50 border-teal-600 text-teal-700' : 'bg-white text-gray-600'}`}
              >
                Long ( 10 min)
              </button>
            </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 py-3 max-w-7xl mx-auto flex justify-between items-center">
        <span className="text-sm text-gray-600">
          {meta?.total || 0} article{meta?.total !== 1 && 's'} trouvé{meta?.total !== 1 && 's'}
        </span>
